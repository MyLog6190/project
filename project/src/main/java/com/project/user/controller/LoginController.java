...
public class LoginController {
	...
	public String kakaoCallback(String code) {
		...
		log.info("서버 아이디(이메일) : " + kakaoProfile.getKakao_account().getEmail());
		log.info("서버 닉네임: " + kakaoProfile.getProperties().getNickname());
		String password = kakaoProfile.getId().toString();
		log.info("서버 패스워드:" + password);
		log.info("서버 성별: " + kakaoProfile.getKakao_account().getGender());
		log.info("서버 프로필 이미지: " + kakaoProfile.getProperties().getProfile_image());
		...
		// 가입자, 비가입자 체크
		User originUser = userService.findUser(kakaoUser.getUser_id());
		// 없는 유저면
		if (originUser == null) {
			log.info("없는 유저입니다.");
		...
		Authentication authentication = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(kakaoUser.getUser_id(), password));
